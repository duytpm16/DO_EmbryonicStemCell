  ...
  function(threshold){
    ...
    for(i in threshold){
        ...
        for(j in 1:nrow(high_peaks)) {
            ...
            this_K <- K[[qtl_chr]]
            this_K <- this_K[names(this_y),names(this_y)]  # Kinship matrix as QTL chromosome
            ...
            this_predict <- genoprobs[[qtl_chr]][,,qtl_marker]                                                      # Get same probabilities in full MUGA array at QTL chromosome and marker 969 x 8
            ...
            # Fit Alt model
            alt_fit <- lmmbygls(y = this_y, X = cbind(this_X_Q_2, this_X_covar), K = this_K[names(this_y), names(this_y)], na.action = na.omit)
