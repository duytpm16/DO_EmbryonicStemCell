...
load("~/Desktop/Munger Embryonic Stem Cells/Overlap/munger_esc_174_mrna_protein_qtl_viewer_v1.RData")
transband <- readRDS("~/Desktop/Munger Embryonic Stem Cells/Proteomics/Viewer/munger_esc_proteomics_transband.rds")
lod.peaks <- dataset.esc.proteins.174$lod.peaks$additive
lod.peaks <- lod.peaks[!lod.peaks$cis,]
expr      <- dataset.esc.proteins.174$data$rz
covar     <- dataset.esc.proteins.174$covar.matrix
med_expr  <- dataset.esc.mrna.174$data$rz
med_annot <- dataset.esc.mrna.174$annot.mrna %>% dplyr::rename(pos = start)
...
for(i in 11:11){
    ...
    print(ggplot(med_melt, aes(x = variable, y = target)) + geom_tile(aes(fill = med_melt$`Proportion Drop`), color = 'black')  + 
            ...
            scale_fill_gradientn(colors = c('red4','orange','yellow')) + ggtitle(paste0('Chr',hs_chr,' HotSpot Interval Mediation')))
